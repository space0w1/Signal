...
--
-- Soft-delete (section 2.9): removing a stock sets is_active=0
...
total_quantity REAL NOT NULL,             -- cumulative shares held
total_cost     REAL NOT NULL,             -- cumulative cost basis (sum of price*qty), native currency
date_added     TEXT NOT NULL,             -- date of first purchase
is_active      INTEGER NOT NULL DEFAULT 1,-- soft-delete flag: 1 = currently held, 0 = removed
removed_date   TEXT,                      -- date soft-deleted; NULL while active
...
source       TEXT,
url          TEXT,
published_at TEXT,             -- article's own publish timestamp, from yfinance
fetched_date TEXT NOT NULL,    -- the cron run date this was pulled on
...
--
-- Note: SQLite treats each NULL as distinct in a UNIQUE
-- constraint, so a NULL ticker wouldn't actually enforce
-- one-row-per-date for the portfolio target. Using the sentinel
-- string '__portfolio__' instead of NULL avoids that trap.
-- ============================================================
...
generated_at   TEXT NOT NULL,      -- timestamp; distinguishes nightly-cron vs on-demand-fallback generation
UNIQUE (user_id, target_type, ticker, date)
);

CREATE INDEX idx_summaries_lookup ON summaries (user_id, target_type, ticker, date);
```