...
| Method | Path | Query params | Returns |
|---|---|---|---|
| GET | `/api/summary` | `target=portfolio\|<ticker>`, `date` (optional, defaults to today) | Summary text via the configured LLM provider (`LLM_PROVIDER` env var: `gemini` [default, `GEMINI_MODEL`, default `gemini-3.7-flash`] or `anthropic` [`claude-opus-5`] — see `backend/app/services/llm/`), grounded in that target's position data and its held tickers' news (`article_text`, best-effort extracted via trafilatura). Prompt templates live as plain text in `backend/app/services/prompts/*.md`, loaded at import time — separate from the data-gathering code, so wording can be tuned by editing markdown, without touching business logic. For `target=portfolio`, also includes `cited_news` — the full news items the model actually cited, resolved from `cited_news_ids` and returned in the model's ranked order (not just IDs). Checks the `summaries` cache first (one row per target+date, never overwritten — the same on-demand fallback pattern as news/price/fx); if missing, generates live, caches it, then returns it. Fails with 503 if the configured provider's API key isn't set or the API call fails. |
